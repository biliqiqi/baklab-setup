...
class SetupApp {
  constructor() {
    ...
    this.developmentMode = window.__BAKLAB_SETUP__?.development === true;
    ...
    this.steps = [
      {
        key: "welcome",
        titleKey: "setup.steps.welcome",
        handler: (container, deps) => InitStep.render(container, deps),
      },
      {
        key: "database",
        titleKey: "setup.steps.database",
        handler: (container, deps) => DatabaseStep.render(container, deps),
      },
      {
        key: "redis",
        titleKey: "setup.steps.redis",
        handler: (container, deps) => RedisStep.render(container, deps),
      },
      {
        key: "smtp",
        titleKey: "setup.steps.smtp",
        handler: (container, deps) => SMTPStep.render(container, deps),
      },
      {
        key: "app",
        titleKey: "setup.steps.application",
        handler: (container, deps) => ApplicationStep.render(container, deps),
      },
      {
        key: "ssl",
        titleKey: "setup.steps.ssl",
        handler: (container, deps) => SSLStep.render(container, deps),
      },
      {
        key: "admin",
        titleKey: "setup.steps.admin_user",
        handler: (container, deps) => AdminStep.render(container, deps),
      },
      {
        key: "oauth",
        titleKey: "setup.steps.oauth",
        handler: (container, deps) => OAuthStep.render(container, deps),
      },
      {
        key: "goaccess",
        titleKey: "setup.steps.goaccess",
        handler: (container, deps) => GoAccessStep.render(container, deps),
      },
      {
        key: "review",
        titleKey: "setup.steps.review",
        handler: (container, deps) => ReviewStep.render(container, deps),
      },
      {
        key: "config_complete",
        titleKey: "setup.steps.config_complete",
        handler: (container, deps) =>
          ConfigCompleteStep.render(container, deps),
      },
    ];
    if (this.developmentMode) {
      this.steps = this.steps.filter((step) => step.key !== "ssl");
    }
  ...
  async init() {
    ...
    try {
      ...
      if (this.developmentMode) {
        this.configStore.set("development", true);
        this.configStore.set("ssl", {
          enabled: false,
          cert_path: "",
          key_path: "",
          use_setup_cert: false,
  ...
  render() {
    ...
    document
      .querySelectorAll(".sidebar-step[data-step-index]")
      .forEach((stepElement) => {
        stepElement.addEventListener("click", () => {
          const stepIndex = parseInt(
            stepElement.getAttribute("data-step-index"),
          );
  ...
  async checkAndLoadImportedConfig() {
    try {
      ...
      if (
        ...
        statusResponse.data &&
        statusResponse.data.revision_mode &&
        statusResponse.data.revision_mode.enabled
      ) {
        console.log(
          "Revision mode detected, loading imported configuration...",
        );
